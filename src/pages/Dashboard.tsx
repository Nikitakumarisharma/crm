...
const Dashboard = () => {
  ...
  const filteredProjects = projects.filter(project => {
    if (user?.role === "sales") {
      ...
      return true; // CTO can see all projects
  ...
  const userProjects = filteredProjects.slice(0, 5); // Most recent 5 projects
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-3xl font-bold flex items-center">
                ...
                {filteredProjects.filter(p => !["completed"].includes(p.status)).length}
              </div>
            </CardContent>
          </Card>
